...
router.post('/bots', requireFields(['token']), async (req, res) => {
...
router.post('/channels/:botId', requireFields(['channelId']), async (req, res) => {
...
router.post('/agents/:botId', requireFields(['number']), async (req, res) => res.json(await Agent.create({ botId: req.params.botId, number: req.body.number })));
...
router.post('/broadcast/:botId', requireFields(['text']), async (req, res) => {
...
router.get('/broadcast/:jobId/stream', async (req, res) => {
  ...
  if (streamToken) {
    try {
      jwt.verify(streamToken, env.jwtSecret);
    } catch (err) {
