...
const runtimeByWebhookPath = new Map();
...
function startKeyboard(settings, channels) {
  const rows = channels.map((c) => {
    const label = c.username || c.title || c.channelId;
    if (c.url) return [Markup.button.url(label, c.url)];
    return [Markup.button.callback(`📌 ${label}`, 'CHECK_JOINED')];
  });
...
async function launchBot(botDoc) {
  ...
  runtimeByWebhookPath.set(botDoc.webhookPath, runtime);
  return runtime;
}
...
async function stopBot(botId) {
  ...
  runtime.bot.stop();
  runtimeByWebhookPath.delete(runtime.webhookPath);
...
function webhookHandler(req, res) {
  const runtime = runtimeByWebhookPath.get(req.path);
